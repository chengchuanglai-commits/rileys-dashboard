...
# ===== 直观图: 同样$2000各配置净值曲线 + 美元盈亏表 =====
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as _plt
from datetime import datetime as _dt
_curves={"QQQ 1x":np.array(legs["QQQ"]),
         "QQQ + Momentum 80/20":np.array(_combo(0.20)),
         "Leveraged 1.5x":np.array(legs["lev(杠杆闸门)"]),
         "Leveraged + Momentum 80/20":np.array(_combo_lev(0.20))}
_ml=min(len(c) for c in _curves.values())
_x=[_dt.strptime(d,"%Y-%m-%d") for d in bt[:_ml]]
_col={"QQQ 1x":"#999999","QQQ + Momentum 80/20":"#2a9d8f",
      "Leveraged 1.5x":"#e76f51","Leveraged + Momentum 80/20":"#1d3557"}
_plt.figure(figsize=(11,6))
for k,c in _curves.items():
    _plt.plot(_x,c[:_ml],label=f"{k}  ->  ${c[-1]:,.0f}",lw=2.2,color=_col[k])
_plt.axhline(INIT,color="#bbb",ls="--",lw=1)
_plt.title("$2000 invested 2021-06 to 2026-06  (backtest, survivorship-biased)",fontsize=12)
_plt.ylabel("Account value (USD)"); _plt.legend(loc="upper left",fontsize=10,framealpha=0.9)
_plt.grid(alpha=0.3); _plt.tight_layout()
_plt.savefig("dashboard/combo-equity.png",dpi=115); print("\n图已存 dashboard/combo-equity.png")
print(f"\n{'='*58}\n  同样 $2000 投入,五年后(回测)\n{'='*58}")
print(f"  {'配置':<26}{'变成':>9}{'净赚':>10}{'最深回撤':>9}")
_dollars={}
for k,c in _curves.items():
