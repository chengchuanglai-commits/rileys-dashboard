import anthropic
import json
import os
from datetime import datetime, timezone, timedelta

client = anthropic.Anthropic()

# Beijing time
beijing_tz = timezone(timedelta(hours=8))
now_beijing = datetime.now(beijing_tz)
today = now_beijing.strftime('%Y-%m-%d')
generated_at = now_beijing.strftime('%Y-%m-%dT%H:%M:00')

response = client.messages.create(
    model="claude-sonnet-4-6",
    max_tokens=4096,
    tools=[{"type": "web_search_20250305", "name": "web_search"}],
    messages=[{
        "role": "user",
        "content": f"""今天是 {today}。请搜索当日最新的美股市场数据，生成金融晨报。

搜索内容：
1. S&P 500 期货今日涨跌幅（百分比，正数表示上涨）
2. 10年期美债收益率及今日变化（单位bps）
3. 当前Q1或最新财报季 EPS 超预期率（%）
4. 今日全市场5支值得关注的股票，覆盖不同板块（科技、能源、医疗、消费、固收各一支）

搜索完成后，只输出以下纯 JSON（不要任何解释文字，不要 markdown 代码块，只要原始JSON）：

{{
  "date": "{today}",
  "generated_at": "{generated_at}",
  "market_status": "pre-market",
  "market": {{
    "sp500_futures_pct": 数字,
    "treasury_10y": 数字,
    "treasury_10y_change_bps": 数字,
    "eps_beat_rate": 数字
  }},
  "note": {{
    "market_overview": "2-3句中文，新手友好，解释期货涨跌和今日焦点",
    "macro": "2-3句中文，关于利率/美联储/地缘政治",
    "earnings": "2-3句中文，关于本财报季整体表现",
    "trade_ideas": "3条建议，每条用⚡开头，中文，简短"
  }},
  "stock_picks": [
    {{
      "ticker": "股票代码",
      "name": "公司名",
      "sector": "板块/细分",
      "direction": "buy或sell或watch",
      "buy_zone": "$X–$Y或—",
      "target": "$X或—",
      "stop_loss": "$X或—",
      "reason": "一句中文，新手友好，说明为何关注"
    }}
  ]
}}

严格要求：
- stock_picks 恰好5支，覆盖科技/能源/医疗/消费/固收不同板块
- direction 只能是 buy、sell、watch 三者之一
- 所有 note 和 reason 字段用中文
- market 中的数字用真实搜索数据，不要虚构
"""
    }]
)

# Extract the text from the last content block
text = ""
for block in response.content:
    if hasattr(block, 'text'):
        text = block.text.strip()

# Parse JSON
start = text.find('{')
end = text.rfind('}') + 1
if start == -1 or end == 0:
    raise ValueError(f"No JSON found in response: {text[:200]}")

data = json.loads(text[start:end])

# Validate
assert len(data['stock_picks']) == 5, f"Expected 5 picks, got {len(data['stock_picks'])}"
for pick in data['stock_picks']:
    assert pick['direction'] in ('buy', 'sell', 'watch'), f"Invalid direction: {pick['direction']}"

# Write morning-note.js
js_content = (
    "// 金融晨报数据 — 每日 07:30 自动更新\n"
    f"window.MORNING_NOTE = {json.dumps(data, ensure_ascii=False, indent=2)};\n"
)

os.makedirs('dashboard', exist_ok=True)
with open('dashboard/morning-note.js', 'w', encoding='utf-8') as f:
    f.write(js_content)

# Write history archive
os.makedirs('dashboard/morning-note-history', exist_ok=True)
archive_path = f'dashboard/morning-note-history/{today}.json'
with open(archive_path, 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print(f"✅ Morning note generated for {today}")
print(f"   S&P futures: {data['market']['sp500_futures_pct']}%")
print(f"   10Y Treasury: {data['market']['treasury_10y']}%")
print(f"   Stock picks: {[p['ticker'] for p in data['stock_picks']]}")
